 ...
 */
public class DataSyncHelperV6 extends JdbcDaoSupport {
	 ...
	 * @param num
	 */
	private void syncTempTable(int num) {
		int max = 0;
	...
	public void calcGroup() {
		...
		try {
			...
			for (Map<String, Object> map : list) {
				...
				// 查询船舱ID
				sql = " SELECT cabin.id, t.task_id AS 'taskId' FROM tab_cabin cabin "
				...
				if (cabinNums == null || cabinNums.isEmpty()) {
					...
					groupId = createGroup(taskId, cabinId, cmsid, operationType, time);
				}

				// 更新表b数据
				try {
					
					sql = " INSERT INTO tab_temp_b_"+ taskId +" (id, Time, Cmsid, PushTime, OneTask, direction, unloaderMove, operationType, offset, groupId) SELECT id, Time, Cmsid, PushTime, OneTask, direction, unloaderMove, operationType, offset, ? from tab_temp_b where id = ? and cmsid = ? ";
					args = new Object[] { groupId, id, cmsid };
					this.getJdbcTemplate().update(sql, args);
					
					sql = " delete from tab_temp_b where id = ? and cmsid = ? ";
	 ...
	 * @return
	 */
	private Integer createGroup(Integer taskId, Integer cabinId, String cmsid, int operationType, Date time) {
		int groupId = 0;
		Object[] args = null;
		// 查询组信息，根据舱ID、 卸船机ID
		String sql = " select * from tab_temp_c_"+ taskId +"  t where t.`status` = 0 and t.cabinId = ? AND t.Cmsid = ? ";
		...
		if (conut == 0) {
			...
			this.getJdbcTemplate().update(new PreparedStatementCreator() {
				public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
					String inster_sql = "";
					if (0 == operationType) {
						inster_sql = " insert into tab_temp_c_"+ taskId +"   (cabinId, Cmsid, startTime, status)  values(?,?,?,?)";
					} else if (1 == operationType) {
						inster_sql = " insert into tab_temp_c_"+ taskId +"   (cabinId, Cmsid, startTime, firstTime, lastTime, status)  values(?,?,?,?,?,?)";
					}
			...
			// 维护上一组结束时间
			sql = " select * from tab_temp_c_"+ taskId +"   t where t.`status` = 0 AND t.Cmsid = ? AND t.id != ? ";
			List<Map<String, Object>> groups = this.getJdbcTemplate().queryForList(sql, cmsid, groupId);
			if (groups != null) {
				for (Map<String, Object> m : groups) {
					Integer id = (Integer) m.get("id");
					if (0 == operationType) {
						sql = " UPDATE tab_temp_c_"+ taskId +"   t SET t.endTime = ?, t.status = 1 WHERE t.id = ? ";
						args = new Object[] { time, id };
					} else if (1 == operationType) {
						sql = " UPDATE tab_temp_c_"+ taskId +"   t SET t.endTime = ?, t.lastTime = ?, t.status = 1 WHERE t.id = ? ";
			...
			case 1:
				if (firstTime == null) {
					sql = " UPDATE tab_temp_c_"+ taskId +"   t SET t.firstTime = ?, t.lastTime = ?  WHERE t.id = ? ";
					args = new Object[] { time, time, groupId };
				} else {
					sql = " UPDATE tab_temp_c_"+ taskId +"   t SET t.lastTime = ?  WHERE t.id = ? ";
	...
	public void updateGroupEndTime(Integer taskId) {
		String sql = " select c2.* from tab_temp_c_"+ taskId +"   c2 LEFT JOIN v_cabin_info c1 ON c1.cabin_id = c2.cabinId where  c2.`status` = 0 AND c1.task_id = ? ";
		...
		for (Map<String, Object> m : list) {
			...
			sql = " UPDATE tab_temp_c_"+ taskId +"   t SET t.endTime = ?, t.status = ? WHERE t.id = ? ";
	...
	public void recomputationCabinGroup(Integer taskId) throws Exception {
		this.stop();
		try {
			String sql = " UPDATE tab_temp_b_"+ taskId +" b SET b.groupId = 0 where b.groupId in "
					+ " ( select c2.id from tab_temp_c_"+ taskId +"   c2 LEFT JOIN v_cabin_info c1 "
					+ "ON c1.cabin_id = c2.cabinId where c1.task_id = ? )";
			this.getJdbcTemplate().update(sql, taskId);

			sql = " DELETE FROM tab_temp_c_"+ taskId +"   t where t.id in "
					+ "( select c2.id from tab_temp_c_"+ taskId +"   c2 LEFT JOIN v_cabin_info c1 ON "
	...
	public static void main(String[] args) throws InterruptedException {
		...
		if (ctx != null) {
			DataSyncHelperV6 helper = (DataSyncHelperV6) ctx.getBean("dataSyncHelper");
			helper.start();
		}
	}
