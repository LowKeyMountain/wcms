...
//	Page<Unloader1> findByCmsid(String cmsId, Pageable pageable);
		...
		return this.findAll(new Specification<UnloaderAll>() {
			public Predicate toPredicate(Root<UnloaderAll> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(StringUtils.isNotEmpty(operationType)) {
					...
					predicates.add(cb.notEqual(root.get("operationType"), "3"));
				}
	 ...
	 * @return
	 */
	default List<UnloaderAll> findListByParams(Map<String, String> params) {

	    List<UnloaderAll> result = this.findAll(new Specification<UnloaderAll>() {
...
/*				predicates.add(cb.greaterThanOrEqualTo(root.get("pushTime"),
	...
	default Page<UnloaderAll> findHistoryAllParam(Pageable pageable,  Map<String, String> params) {
		return this.findAll(new Specification<UnloaderAll>() {
			public Predicate toPredicate(Root<UnloaderAll> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				Predicate predicate = null;
				List<Predicate> predicates = new ArrayList<Predicate>();
				...
				predicates.add(cb.equal(root.get("operationType"), "3"));
				if("1".equals(cmsId)) {
					predicates.add(cb.equal(root.get("cmsId"), "ABB_GSU_1"));
				} else if ("2".equals(cmsId)) {
					predicates.add(cb.equal(root.get("cmsId"), "ABB_GSU_2"));	
